/**
 * ABOUTME: Create home — names the home entity before sensor pairing.
...
export default function CreateHomeScreen({ navigation }: Props) {
  ...
  const { user, addLinkedHome } = useAuthStore();
  ...
  const handleCreate = () => {
    ...
    setTimeout(() => {
      ...
      addHome({ id: homeId, name: homeName.trim(), createdBy: user?.id || 'unknown', createdAt: new Date().toISOString() });
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
      <KeyboardAvoidingView style={styles.kav} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.content}>
          ...
          <Text style={[styles.title, { color: colors.text }]}>Name your home</Text>
          ...
          <Button title="Continue →" onPress={handleCreate}
            disabled={homeName.trim().length < 2} loading={loading} />
        </View>
